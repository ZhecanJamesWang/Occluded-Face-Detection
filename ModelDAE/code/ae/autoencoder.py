...
class AutoEncoder(object):
  ...
  @staticmethod
  def _activateLR(x, w, b, transpose_w=False):
  ...
  def supervised_net(self, input_pl):
    ...
    last_output = self._activateLR(last_output, w, b)
  ...
  with ae.session.graph.as_default():
    ...
    for step in xrange(steps):
      ...
      if (step + 1) % 1000 == 0 or (step + 1) == steps:
        print ("insdie testing area: ", step)
    ...
    steps_per_epoch = data.test.num_examples // FLAGS.batch_size
